    ...
    for key, value in pairs:
        if key in result:
            raise WorkspaceAccessInvalidError(
                "Project access sidecar contains duplicate fields"
            )
        result[key] = value
    return result
...
def read_workspace_owner(workspace_path: Path) -> str:
    ...
    try:
        ...
        if is_link_or_reparse(path_metadata):
            raise WorkspaceAccessInvalidError("Project access sidecar is unsafe")
        descriptor = os.open(path, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > _MAX_ACCESS_BYTES:
            raise WorkspaceAccessInvalidError("Project access sidecar is invalid")
        content = bytearray()
    ...
    if not isinstance(payload, dict) or set(payload) != {"owner_id"}:
        raise WorkspaceAccessInvalidError("Project access sidecar is noncanonical")
    owner_id = payload["owner_id"]
    if not isinstance(owner_id, str):
        raise WorkspaceAccessInvalidError("Project owner is invalid")
    try:
        return validate_user_id(owner_id)
    except InvalidInputError as exc:
        raise WorkspaceAccessInvalidError("Project owner is invalid") from exc
