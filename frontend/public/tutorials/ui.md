...
- Each column header shows the column name and its data type (e.g. `datetime`, `date`, `text`, `decimal`; `date` is a calendar date with no time of day, useful for publication or sitting dates). Click the settings icon on a column to rename or delete it; use the data-type menu to convert its type. These operations update the selected Data Block without creating a new one. Values that cannot be converted (for example words in a column changed to integer) become empty; if that happens to a large share of the column, a warning says how many, and **Undo** restores them while the Project is open. When converting, the app attempts to guess the date format automatically. This works for many common formats but can fail or produce incorrect results when the format is ambiguous (e.g. `01/02/03` could be read as DD/MM/YY, MM/DD/YY, or YY/MM/DD). If the conversion fails or the dates look wrong, use the **Format** field to specify the format explicitly using [Python strftime/strptime codes](https://docs.python.org/3/library/datetime.html#strftime-and-strptime-format-codes). Common examples:
  - `%Y-%m-%d` → `2025-05-06`
  - `%d/%m/%Y` → `06/05/2025`
  - `%m/%d/%Y` → `05/06/2025`
