...
describe('QuotationNodeBlock', () => {
  ...
  it('keeps document cells mounted when the highlight hover state changes', () => {
    const row = normalizeQuotationRow(
      {
        text: 'Alice said hello.',
        [QUOTATION_COLUMN_KEYS.quote]: 'hello',
    ...
    const nodeBlock = (hoverState: QuotationHoverState | null) => (
      <QuotationNodeBlock
        nodeId="node-1"
        textCol="text"
        cols={[QUOTATION_DOCUMENT_COLUMN]}
    ...
    // Hovering a highlight lifts hover state to the feature, which re-renders
    // the block. Remounting the cell would reset its hover tooltip.
    rerender(nodeBlock({ key: 'node-1:0:document', segmentStart: 11, type: 'quote' }));

    expect(screen.getByTestId('quotation-clamped-cell')).toBe(cell);
  });
});
